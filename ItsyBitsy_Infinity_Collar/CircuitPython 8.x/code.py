...
# Bluetooth
ble = BLERadio()
ble.name = "LED Infinity Collar v2.a"  # Set a custom name for the BLE device
            ...
            if uart_service.in_waiting:
                ...
                if isinstance(packet, ButtonPacket) and packet.pressed:
                    if packet.button == ButtonPacket.BUTTON_1:
                        ...
                        random_color_mode = True
                        print(
                            "button 1 pressed: Animation colors change to a random new value after every animation sequence"
                        )
                    elif packet.button == ButtonPacket.BUTTON_2:
                        # Animation colors stay the same unless manually changed
                        random_color_mode = False
                        print(
                            "button 2 pressed: Animation colors stay the same unless manually changed"
                        )
                        ...
                        print(
                            "button left pressed: Go to first animation in the sequence"
                        )
                    elif packet.button == ButtonPacket.RIGHT:
                        # Go to the next animation in the sequence
                        animations.next()
                        print(
                            "button right pressed: Go to next animation in the sequence"
                        )
